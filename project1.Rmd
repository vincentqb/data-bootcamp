...
sec12 <- read.xlsx("project1_data/Fall 2015 Lab 120, Sec 1L12.xlsx", sheetName = "Sheet1", rowIndex = -c(1:7), colIndex = c(1:15))
summary(sec12)
...
```
sec09 <- read.ods("project1_data/Fall 2015 Lab 120 section 1L09.ods", sheet = 1)
...
```
lec <- read.xlsx("project1_data/lecture.xlsx", sheetName = "Sheet1", colIndex = c(2:5))
...
We tidy the data.
FIXME: Don't tidy too much. Keep columns.
```
data <- data %>% 
...
?summarise_each
data %>% group_by(ID, Type) %>% summarise_each(funs(mean))

minmax <- function(x){
  .20*min(x) + .35*max(x)
}
data[data$Type == "Midterm",] %>% group_by(ID, Type) %>% mutate(Score = minmax(Score))
```

We find out who has at least 10 labs, and add as a new column.
```
count_lab <- data %>%
  filter(Type == "Lab" & Score > 0) %>%
  count(ID, Type) %>%
  select(ID, n)
  
names(count_lab) <- c("ID","EnoughLab")
count_lab$EnoughLab <- count_lab$EnoughLab >= 10

data <- merge(data, count_lab)
...
data <- data %>%
  group_by(ID, Type) %>%
  mutate(MeanType = mean(Score))

data.mean <- data
```

We now find the min and max midterm.
```
data <- data %>%
  group_by(ID, Type) %>%
  mutate(MinType = min(Score)) %>%
  mutate(MaxType = max(Score))
```

We no longer need the score column.
```
data <- data %>%
  select(-`Score`) %>%
  distinct()
