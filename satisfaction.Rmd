...
Our goal is to predict employee satisfaction using the survey.
...
Employee and job satisfaction are found in q42 and q43, respectively. We create a new column for a binary classifier that we will predict.
```{r}
# Classify the respondent as happy if they clearly indicated so in the survey
happy_map <- function(x) {
  if (x == "Completely satisfied" || x == "Mostly satisfied") {
    return(TRUE) 
  } else {
    return(FALSE)
  }
...
happy_map <- Vectorize(happy_map)
dataset$happy <- happy_map(dataset$q42)
dataset <- dataset %>% select(-c(q42,q43))
```

We need to sanitize the names of the categories we want to predict.
```{r}
...
```{r}
thresold_na <- 1000
dataset.thresold <- dataset[,colSums(is.na(dataset)) < thresold_na]
dataset <- cbind(dataset.thresold, dataset[,"happy"])
# dataset <- cbind(dataset.thresold, dataset[,c("happy", "q42","q43")])
```
...
trainIndex <- createDataPartition(dataset$happy, p = .8, list = FALSE)
...
```{r}
pos <- which(sapply(dataset, is.factor))
...
We remove the users that have too many NAs.
```{r}
thresold.na <- 50
...
rtTune <- train(happy ~ ., data = training, method = "rpart", tuneGrid = rtGrid, trControl = ctrl)
...
thresold.predict <- 0.5
rtPredict$no <- thresold.predict
rtPredict <- rtPredict %>%
  mutate(predict = ifelse(yes > thresold.predict, TRUE, FALSE))

par(mfrow = c(1,1))
plot(rtPredict)

rtCM <- confusionMatrix(rtPredict, datasetTest$happy)
rtCM
