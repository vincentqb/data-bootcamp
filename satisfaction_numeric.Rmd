...
```{r}
svmModel <- svm(q42 ~ ., data = training, cost = 100, gamma = 1)
svmPredict <- predict(svmModel, testing)
# table(predict = svmPredict, true = testing[,"q42"])
table(predict = svmPredict, true = testing$q42)

svmModel

rtCM <- confusionMatrix(svmPredict, testing$q42)
rtCM
```
...
set.seed(1)
enetTune <- train(q42 ~ ., data = training,
                  method = "glmnet", # can replace by "lasso" 
                  tuneGrid = enetGrid,
                  trControl = ctrl)
...
pr_enet <- predict(enetTune, newdata = testing)
RMSE(pr_enet, testing$q42)
```
