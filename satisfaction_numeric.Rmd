---
title: "Predicting Job Satisfaction"
...
We convert q42, q43, and q45 to numeric.
```{r}
# cols <- c(96:97, 99:108)
# dataset[,cols] <- sapply(dataset[,cols], as.numeric)
```
...
```{r}
# lvls <- c(make.names(levels(as.factor(dataset$q42))), "NA.")
# dataset$q42 <- as.factor(make.names(dataset$q42))
# levels(dataset$q42) <- lvls
```
...
```{r}
thresold.na <- 1000
q42 <- dataset %>% select(q42)
dataset.thresold <- dataset %>% select(-q42)
dataset.thresold <- dataset.thresold[,colSums(is.na(dataset.thresold)) < thresold.na]
dataset <- cbind(dataset.thresold, q42)
```
...
```{r}
dataset.thresold <- dataset[,colSums(is.na(dataset)) < 1000]
